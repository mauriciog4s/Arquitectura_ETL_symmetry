 ...
 *   instalar()              → prepara las hojas, BigQuery y el disparador. Se puede repetir sin riesgo.
 *   procesar()              → corre el ETL ahora (el disparador lo hace solo cada 10 minutos).
...
const CONFIG = {
  ...
  MINUTOS_ENTRE_EJECUCIONES: 10, // Frecuencia del disparador (1, 5, 10, 15 o 30)
...
function instalar() {
  ...
  try {
    ...
    ScriptApp.getProjectTriggers().filter(t => t.getHandlerFunction() === 'procesar').forEach(t => ScriptApp.deleteTrigger(t));
    ScriptApp.newTrigger('procesar').timeBased().everyMinutes(CONFIG.MINUTOS_ENTRE_EJECUCIONES).create();
    Log.info('SISTEMA', `Disparador automático activo: cada ${CONFIG.MINUTOS_ENTRE_EJECUCIONES} minutos`);
...
/** Corazón del ETL. Lo llama el disparador cada 10 minutos. */
function procesar() {
  const lock = LockService.getScriptLock();
  if (!lock.tryLock(5000)) return 'Ya hay otra ejecución en curso; se omite esta.';
  ...
  try {
    validarConfig_();
    resumen.archivos_nuevos = detectarArchivos_();
    for (let i = 0; i < 20; i++) {
    ...
    resumen.mensaje = pendientes ? `${pendientes} carga(s) pendiente(s); continúa en la próxima ejecución`
      : (resumen.cargas_trabajadas ? 'Cargas al día' : 'Sin archivos nuevos');
    props.deleteProperty('ULTIMO_ERROR_GENERAL');
  } catch (e) {
    resumen.resultado = 'ERROR';
    resumen.mensaje = e.message;
    // No repetir el mismo error general en el Log/correo cada 10 minutos
    if (props.getProperty('ULTIMO_ERROR_GENERAL') !== e.message) {
      ...
      props.setProperty('ULTIMO_ERROR_GENERAL', e.message);
    }
  } finally {
    Log.flush();
  ...
  return `${resumen.resultado}: ${resumen.mensaje}`;
}
...
function reintentarCarga(idCarga) {
  ...
  try {
    ...
    return 'Reintento programado. Se procesará en la próxima ejecución (o pulse "Procesar ahora").';
  } finally {
    Ctx.reg = null;
    Log.flush();
    lock.releaseLock();
...
function detectarArchivos_() {
  ...
  let recibidos = 0;
  nuevos.forEach(f => {
    if (Date.now() - f.getLastUpdated().getTime() < CONFIG.MINUTOS_ESPERA_ARCHIVO * 60000) return; // aún subiendo
    const reg = {
  ...
  try {
    while (!TERMINALES.includes(reg.estado)) {
      if (tiempoRestante_() < 45000) {
        Log.info(ETAPA_DE_ESTADO[reg.estado] || 'SISTEMA', 'Pausa por tiempo; continúa automáticamente en la próxima ejecución');
        return false;
      }
      let listo;
...
/** Errores fatales → ERROR. Errores temporales → se reintenta en la próxima ejecución (hasta MAX_INTENTOS). */
function manejarFallo_(reg, e, estadoAntes) {
  ...
  Log.aviso(etapa, `Intento ${reg.intentos}/${CONFIG.MAX_INTENTOS} falló; se reintenta en la próxima ejecución: ${e.message}`, detalle);
  Registro.guardar(reg);
...
function pasoOrganizar_(reg) {
  ...
  while (desde < n) {
    if (tiempoRestante_() < 60000) {
      Log.info(etapa, `Escritas ${desde} de ${n} filas en el Sheet; continúa en la próxima ejecución`);
      return false;
    }
...
function pasoEnviar_(reg, recienLanzado) {
  ...
  if (reg.estado === E.ENVIANDO && reg.job_id) {
    ...
    if (!r.terminado) { Log.info(etapa, 'BigQuery sigue recibiendo la carga; se revisa en la próxima ejecución'); return false; }
    reg.job_id = '';
...
function pasoConsolidar_(reg, recienLanzado) {
  ...
  if (reg.estado === E.CONSOLIDANDO && reg.job_id) {
    ...
    if (!r.terminado) { Log.info(etapa, 'BigQuery sigue consolidando; se revisa en la próxima ejecución'); return false; }
    reg.job_id = '';
...
function obtenerDatosPortal() {
  ...
  return {
    generado: ahora_(),
    config: {
      proyecto: CONFIG.PROJECT_ID, dataset: CONFIG.DATASET, minutos: CONFIG.MINUTOS_ENTRE_EJECUCIONES,
    ...
    automatizacion: ScriptApp.getProjectTriggers().some(t => t.getHandlerFunction() === 'procesar'),
...
function validarConfig_() {
  if (!CONFIG.PROJECT_ID || /PEGAR-AQUI/i.test(CONFIG.PROJECT_ID)) {
    throw errorFatal_('Falta poner el ID del proyecto de Google Cloud en CONFIG.PROJECT_ID (Code.gs, línea ~24).');
  }
...
function notificar_(asunto, cuerpo) {
  try {
