...
```
Carpeta Drive (CSV) ─► Apps Script (cada 10 min) ─► Sheet "Transaccional" ─► BigQuery staging ─► BigQuery transacciones
                                   │
...
1. **Code.gs:** borra lo que haya y pega el contenido de `apps_script/Code.gs`. En la línea 24 reemplaza `PEGAR-AQUI-ID-DEL-PROYECTO` por el ID que copiaste en el paso 1.
...
3. Al terminar, el Sheet tendrá las hojas `Transaccional`, `Control_Archivos`, `Log` y `Errores`. BigQuery tendrá el dataset `symmetry` con 3 tablas, y quedará programada una ejecución automática cada 10 minutos. En la hoja **Log** verás todo lo que hizo `instalar`.
...
### Paso 6. Probar
Sube un CSV a la carpeta de entrada y pulsa **Procesar ahora** en el portal, o espera un máximo de 10 minutos. Con el archivo de ejemplo (166.241 filas crudas) el resultado esperado es **49.041 transacciones** cargadas.

---

## Cómo funciona

Cada archivo pasa por 6 pasos. El estado de cada archivo queda guardado en la hoja `Control_Archivos`. Si Google corta la ejecución (Apps Script tiene un límite de 6 minutos), el proceso continúa solo en la ejecución siguiente, desde donde quedó.
...
|---|---|
| Algunas filas con fecha o id inválido (menos del 1 %) | Carga las válidas. Las malas quedan en la hoja **Errores** con la fila original. |
| Más del 1 % de filas malas, o faltan columnas | No carga el archivo, lo marca `ERROR`, lo mueve a `CON_ERROR` y envía un correo. |
| Falla temporal de Google (timeout, cuota) | Reintenta solo, hasta 4 veces por archivo. |
...
| Falta el PROJECT_ID, los permisos o la facturación | Registra un solo aviso con la instrucción para corregirlo (no se repite cada 10 minutos). |
...
- **Correo:** avisa cuando una carga termina bien y cuando hay un error.
...
| `MINUTOS_ENTRE_EJECUCIONES` | `10` | Frecuencia de revisión de la carpeta. Si lo cambias, vuelve a ejecutar `instalar()` |
